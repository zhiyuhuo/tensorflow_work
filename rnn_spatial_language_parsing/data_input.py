...
def get_lm_input_data(N):
	...
	tuple_mat_chunk_input = tuple_mat_chunk_input + (mat_chunk_input,)
	...
	tuple_mat_words_input = tuple_mat_words_input + (mat_word_input,)
        
        mat_label_output = np.zeros((D_label), dtype=np.float64)
        for d in range(D_label):
	    mat_label_output[d] = vec_label[d]
	#print(mat_label_output)
	tuple_mat_label_output = tuple_mat_label_output + (mat_label_output,)
	
    return tuple_mat_chunk_input, tuple_mat_words_input, tuple_mat_label_output
