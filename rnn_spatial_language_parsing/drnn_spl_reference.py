...
n_classes = 8 # linear sequence or not

trainset = LanguageSequenceData('train', 'reference')
testset = LanguageSequenceData('test', 'reference')
...
if __name__ == "__main__":
	...
	with tf.Session() as sess:
	    ...
	    save_path = saver.save(sess, "./model_ref.ckpt")
	
    
    saver = tf.train.Saver(write_version=tf.train.SaverDef.V2) 
    with tf.Session() as sess:
        batch_x, batch_y, batch_seqlen = testset.next(818)
        saver.restore(sess, "./model_ref.ckpt")
	...
	save_file = open('test_reference.out', 'w')
	for r in res:
            save_file.write("%d " % r)
        save_file.write("\n")
